...
namespace Galaxy.Taurus.ShopInfoAPI.Util
{
    ...
    { 
        ...
        //同步POST JSON数据请求
        public string HttpPostJSON(string url, string postData, Dictionary<string, string> headers = null, int timeout = 0, Encoding encoding = null)
        {
            return HttpPost(url, postData, headers, "application/json", timeout, encoding);
        }

        //异步POST JSON数据请求
        public string HttpPostJSONAsync(string url, string postData, Dictionary<string, string> headers = null, int timeout = 0, Encoding encoding = null)
        {
            return HttpPostAsync(url, postData, headers, "application/json", timeout, encoding).Result;
        }
        ...
        /// <param name="encoding">默认UTF8</param>
        /// <returns></returns>
        public async Task<string> HttpPutAsync(string url, string putData, Dictionary<string, string> headers = null, string contentType = null, int timeout = 0, Encoding encoding = null)
        {
            using (HttpClient client = new HttpClient())
            {
                if (headers != null)
                ...
                using (HttpContent content = new StringContent(putData ?? "", encoding ?? Encoding.UTF8))
                {
                    if (contentType != null)
                    {
                        content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue(contentType);
                    }
                    using (HttpResponseMessage responseMessage = await client.PutAsync(url, content))
                    {
                        Byte[] resultBytes = await responseMessage.Content.ReadAsByteArrayAsync();
                        return Encoding.UTF8.GetString(resultBytes);
                    }
